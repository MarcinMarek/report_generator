...
ql = [
    ...
    "plr == plr or plate == plate",   # at least one not nan                             -Total
      ...
      "(plr == plr or plate == plate) and (ctr == ctr or country == country)", #at least one not nan plate and at least one not nan country
      "plate == plr and country == ctr",  # ok, both not nan
      "(plate != plr or country != ctr) and (ctr == ctr or country == country) and (plr == plr or plate == plate)",  # at least one fail, at least one not nan plate and at least one not nan country
      "(plate != plr and country == ctr) and (ctr == ctr or country == country) and (plr == plr or plate == plate)",  # fail text, at least one not nan plate and at least one not nan country
      "(plate == plr and country != ctr) and (ctr == ctr or country == country)",  # fail state, at least one not nan plate and at least one not nan country
      "(plate != plr and country != ctr)  and (plr == plr or plate == plate) and (ctr == ctr or country == country)",  # fail both
      ]]
...
# The conf list idnicates the confidence level we are looking at by the n argument
...
def insert_perc(df): # used in ARH evaluation by confidence levels
    ...
    df.insert(2, "%", [str(round(df['Success'][i] / s * 100, 1))+'%' for i in range(df['Success'].size)], allow_duplicates=True)
    df.insert(4, "%", [str(round(df['-Fail'][i] / s * 100, 1))+'%' for i in range(df['Success'].size)], allow_duplicates=True)
    df.insert(6, "%", [str(round(df['GT!=NA&ARH!=NA'][i] / s * 100, 1))+'%' for i in range(df['Success'].size)], allow_duplicates=True)
    df.insert(8, "%", [str(round(df['GT=NA&ARH!=NA'][i] / s * 100, 1))+'%' for i in range(df['Success'].size)], allow_duplicates=True)
    df.insert(10, "%", [str(round(df['GT!=NA&ARH=NA'][i] / s * 100, 1))+'%' for i in range(df['Success'].size)], allow_duplicates=True)

def append_automation(df, n = 0): # used in inverse sum
    df['#Automation'] = df['-Total'] - df["GT!=NA&ARH=NA"]
    df['%Automation'] = [round(df['#Automation'][i] / df['-Total'][0] * 100, 1) if df['#Automation'][i] != 0 else 0 for i in range(df['#Automation'].size)]

    if n == 0:
        df['#FP'] = df["GT=NA&ARH!=NA"] + df["GT!=NA&ARH!=NA"]
    else:
        df['#FP'] = df["-Fail"]
    df['%FP'] = [round(df['#FP'][i] / df['#Automation'][i] * 100, 1) if df['#FP'][i] != 0 and df['#Automation'][i] != 0 else 0 for i in range(df['#FP'].size)]
...
gc1 = [pd.concat([gc[i], pd.DataFrame({'Total': [gc[i][j].sum() for j in gc[0].columns]}, index=index).transpose() ]) for i in range(3)]
for i in range(3):
    insert_perc(gc1[i])
####
# Cumulative sum of images by the confidence level
gc2 = [gc[i].cumsum() for i in range(3)]
gc2 = [pd.concat([gc2[i], pd.DataFrame({'Total': [gc2[i][j].max() for j in gc[0].columns]}, index=index).transpose()]) for i in range(3)]
for i in range(3):
    gc2[i]['-Total'] = gc1[i]['-Total'].copy()
####
# Global inverted cumulative sum
gc3 = [gc[i][::-1].cumsum()[::-1] for i in range(3)]
gc3 = [pd.concat([gc3[i], pd.DataFrame({'Total': [gc3[i][j].max() for j in gc[0].columns]}, index=index).transpose()]) for i in range(3)]
for i in range(3):
...
#####################################
## XlsxWriter Sheet inicialization ##
#####################################
...
def create_pdf(arh, gc, inv, title = 'CARACTERES'):
    ...
    c.drawCentredString(pwidth / 2, 780, 'Resumo de resultados')
...
create_pdf(arh[0], gc1[0], gc3[0], 'CARACTERS')
